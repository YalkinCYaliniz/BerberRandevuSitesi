        ...
        public async Task<IActionResult> IndexAsync(CreateViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // Eğer hata yoksa, yeni kullanıcı oluşturulur.
                    if (ModelState.ErrorCount == 0)
                    {
                        ...
                        if (result.Succeeded)
                        {
                            return RedirectToAction("Index", "Login"); // Başarılı kullanıcı oluşturma sonrası giriş yap sayfasına yönlendirme
                        }
            ...
            // Hata durumu veya geçersiz modelde sayfa tekrar gösterilir.
            return View(model);
        }
    }
}